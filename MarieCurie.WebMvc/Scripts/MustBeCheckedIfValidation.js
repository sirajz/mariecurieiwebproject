jQuery.validator.addMethod('mustbecheckedif', function (value, element, params) {
    ...
    if (testValue) {
        if (testPropertyValue != testValue || (testPropertyValue == testValue && $(element).prop('checked')))
            return true;
    }
    ...
    if (testValues) {
        ...
        for (var i = 0; i < testValuesSplit.length; i++) {
            if (testPropertyValue == testValuesSplit[i] && !$(element).prop('checked')) {
                return false;
            }
        }
...
jQuery.validator.unobtrusive.adapters.add('mustbecheckedif', ['propertyname', 'propertyvalue', 'propertyvalues'], function (options) {
    options.rules['mustbecheckedif'] = options.params;
    options.messages['mustbecheckedif'] = options.message;
});
